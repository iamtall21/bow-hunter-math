...
import { generateQuestion, getWrongAnswerFeedback } from '../data/mathTemplates'
import './SpeechBubbleMath.css' // reuse same styles

export default function ForageMathBubble({ screenX, screenY, onCorrect, onWrong }) {
  const state = useGameStore()
  // Use an easier tier to reinforce mastered concepts
  // Foraging questions are always easy — tier 1 reinforcement
  const [question] = useState(() => generateQuestion(1, state.mathTopic))
  const [answer, setAnswer] = useState('')
  const [result, setResult] = useState(null)
  const inputRef = useRef(null)
  const timerRef = useRef(null)
  const [timeLeft, setTimeLeft] = useState(30) // generous timer
  ...
  return (
    <div className="speech-bubble-anchor" style={bubbleStyle}>
      <div className="speech-bubble">
        {!result ? (
          <>
            <div className="bubble-animal-name" style={{ color: '#8bc48b' }}>
              GATHER MATERIALS
            </div>
            {state.timerEnabled && (
          ...
          <div className="bubble-result">
            {result === 'correct' ? (
              <div className="bubble-result-text correct">GATHERED!</div>
            ) : (
              <>
                <div className="bubble-result-text wrong">FUMBLED!</div>
